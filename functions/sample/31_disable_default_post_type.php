...
function remove_default_taxonomy()
{
    ...
    if (!empty($wp_rewrite->queryreplace[array_keys($wp_rewrite->queryreplace, 'category_name=')[0]])) unset($wp_rewrite->queryreplace[array_keys($wp_rewrite->queryreplace, 'category_name=')[0]]);
    if (!empty($wp_rewrite->queryreplace[array_keys($wp_rewrite->queryreplace, 'tag=')[0]])) unset($wp_rewrite->queryreplace[array_keys($wp_rewrite->queryreplace, 'tag=')[0]]);
    if (!empty($wp_rewrite->rewritecode[array_keys($wp_rewrite->rewritecode, '%category%')[0]])) unset($wp_rewrite->rewritecode[array_keys($wp_rewrite->rewritecode, '%category%')[0]]);
    if (!empty($wp_rewrite->rewritecode[array_keys($wp_rewrite->rewritecode, '%post_tag%')[0]])) unset($wp_rewrite->rewritecode[array_keys($wp_rewrite->rewritecode, '%post_tag%')[0]]);
